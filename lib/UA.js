  ...
  if(!request.to_tag) {
    switch(method) {
      ...
      case JsSIP_C.ACK:
        ...
        break;
      case JsSIP_C.NOTIFY:
        if (this.listeners('newNotify').length === 0) {
          request.reply(405);
        } else {
          request.reply(200);
...
UA.prototype.loadConfig = function(configuration) {
  ...
  this.contact = {
    pub_gruu: null,
    temp_gruu: null,
    uri: new URI('sip', Utils.createRandomToken(8), settings.via_host, null, {transport: settings.ws_servers && settings.ws_servers[0] && settings.ws_servers[0].scheme === 'WSS' ? 'wss' : 'ws'}),
