    ...
    switch(request.method) {
      ...
      case JsSIP_C.INFO:
        if(this.status === C.STATUS_CONFIRMED || this.status === C.STATUS_WAITING_FOR_ACK || this.status === C.STATUS_INVITE_RECEIVED) {
          contentType = request.getHeader('content-type');
          if (request.getHeader('X-UA-EX')) {
            this.emit('receivedInfoXUaEx', { originator: 'remote', request: request });
            request.reply(200);
          }
          else if (contentType && (contentType.match(/^application\/dtmf-relay/i))) {
...
function receiveNotify(request) {
  ...
  if (typeof request.event === undefined) {
    request.reply(400);
  } else if (request.event.event === 'session-info' && this.listeners('notifiedSessionInfo').length !== 0) {
    this.emit('notifiedSessionInfo', { originator: 'remote', request: request });
    request.reply(200);
